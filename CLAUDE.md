...
- **`MareyComponent`** — Space-time diagram. `getPolylinePath()`/`collectSegmentPoints()` both derive from a shared private `walkSegments()` walk that lifts the pen (starts a new segment) on an unmapped position or a jump bigger than `maxJumpDistance`; `collectSegmentPoints()` is memoized per `coordinates` array identity in `segmentPointsCache` (a `WeakMap`, safe because `trainRuns`/`plannedRuns` arrays are only ever replaced wholesale, never mutated) since template bindings call the marker methods several times per change-detection cycle. Pen-up/pen-down points are rendered as triangle markers by `getSpawnMarker`/`getInternalBreakPoints`/`getActualEndMarker` (actual trajectories) and `getPlannedStartMarker`/`getPlanTargetMarker` (planned routes) — whether a marker is single/double/absent encodes spawn vs. mid-route gap vs. a genuine divergence between the actual trajectory and its plan at the NOW seam. Draws a green (`#2e9e44`) NOW line at `timestep` (`= history.length`) always, and, when replaying, a red (`#c0392b`) REPLAY/FUTURE line at `replayTime` (label is "FUTURE" once `replayTime > timestep`), matching `ReplayBadgeComponent`'s LIVE/REPLAY/FUTURE coloring. `plannedRuns` (from `getPlans()`) render as dash-dotted per-agent routes, hue-matched to the same agent's actual-trajectory color via `getTrainHueIndex()`; `getPlanLabels()` groups route-endpoint labels by pixel position so agents sharing an identical source+target get one merged `"id/id/id"` label instead of stacked overlapping ones.
- **`ReplayBadgeComponent`** (`app-replay-badge`) — Small standalone LIVE/REPLAY/FUTURE pill, self-positioned top-right via `:host { position: absolute }` (host container must be `position: relative`); reads `StateService.getReplayTime()`/`getHistory()` directly, no `@Input`s — `future` is true once `replayTime > history.length`. Used in `MapComponent` and `MareyComponent`, not in `LinkMapComponent`.

**Time machine / replay:**

`StateService.replayTime` (`number | null`, `null` = live) holds a step count in the same numbering as `state.steps`/`history.length` — i.e. 1-based, not a 0-based array index. `getDisplayedAgents()` combines live `agents`, `history`, `replayTime`, and `plans`: when replaying it returns `Object.values(history[replayTime - 1])`; if that snapshot doesn't exist (browsing past NOW into the FUTURE) it falls back to `Object.values(plans[0][replayTime - 1])`; if that's also missing, it falls back to live agents. `getMaxReplayTime()` is the time machine's upper bound — `max(history.length, ...plans.map(p => p.length))` — so the time machine can browse into the predicted future once a loaded plan reaches further than the actual history. `MapComponent.enterTimeMachine()` seeds `replayTime` with `Math.max(1, nowTime)` (not bare `nowTime`, which can still be 0 if a plan loaded before the first Step — see `maxReplayTime` below); `timeMachinePrev()`/`timeMachineNext()` clamp to `[1, maxReplayTime]`; `leaveTimeMachine()` sets it back to `null`. The ENTER button is disabled only via `maxReplayTime === 0` (not `nowTime === 0`), since a plan can make time-machine browsing available before the first Step. New steps arriving while replaying do **not** auto-advance `replayTime` — the view stays frozen at the chosen step until the user moves it or leaves.
...
Frontend: `frontend/src/app/{data,state,controller}.service.spec.ts` (Karma/Jasmine, run via `npm test`) cover the agent-plans and time-machine wiring described above — `DataService.getTrajectoryAgentPlans` HTTP shape, `StateService.getDisplayedAgents`/`getMaxReplayTime`/`setPlans`/`clearHistory` reactive behavior (including the FUTURE-plan fallback), and `ControllerService`'s `reset()`/step-loop orchestration (mocking `DataService`/`StateService` via `jasmine.createSpyObj`, since `ControllerService`'s constructor auto-triggers `reset()` once `getEnvs()`/`getPolicies()` resolve non-empty — keep those spies resolving to `[]` in tests that don't want that). `marey/marey.component.spec.ts` and `replay-badge/replay-badge.component.spec.ts` are the first component-level specs in the repo — since the tested logic (Marey's marker-computation methods; the replay badge's LIVE/REPLAY/FUTURE state) doesn't depend on template rendering, both construct the component directly (`new MareyComponent({} as StateService)` / `new ReplayBadgeComponent(new StateService())`) and set public fields or call `ngOnInit()` manually instead of going through `TestBed.createComponent()` — no fixture, no change detection. Follow the service specs' `jasmine.createSpyObj`/`TestBed` pattern for anything HTTP- or DI-heavy, and this direct-instantiation pattern for pure component logic that doesn't need the DOM.
...
- **`/agent_plans` returns at most one plan**: `_build_agent_plans_content` returns `[]` or `[plan]` — never more than one — so `StateService.plans`/`MareyComponent.plannedRuns` are typed as arrays for HTTP-shape reasons only, not because there's a multi-plan feature to select between. Treat `plans[0]`/`plannedRuns[0]` as "the loaded plan", not index 0 of several; don't reintroduce a per-plan selection mechanism without also building the UI to drive it (a previous one was dead code — `StateService.plan`/`getPlan()`, never emitted to by anything — and was removed).
